...
public class Interfaz_ppal_beta {
	...
	private void initialize(final Matriz juego) {
		...
		// Control de teclado lo saque de lo de la profe
	...
	private void dibujar(Matriz juego) {
		for (int i = 0; i < matriz.length; i++) {
			for (int j = 0; j < matriz.length; j++) {
				int numero = juego.ObtenerElem(i, j);
				...
				switch (numero) {
					case 0:
						matriz[i][j].setText("");
						matriz[i][j].setBackground(Color.LIGHT_GRAY);						
						break;
					case 2:
						matriz[i][j].setText(String.valueOf(numero));
